...
namespace RPG.GlobalModule.View
{
    public class RewardUI : MonoBehaviour
    {
        ...
        /// </summary>
        /// <returns></returns>
        public List<Choices> getSelectedRewards()
        {
            if (!_displayPlayerElements)
                _playerElementMenu.manualyCheckSelectedElement();

            List<Reward> choosedPlayerElements = getSelectedReward(_playerElementMenu);
            List<int> choosedPlayerElementsIndex = getSelectedRewardIndexes(_playerElementMenu);

            List<Reward> choosedRewards = getSelectedReward(_rewardsElementMenu);
            List<int> choosedRewardsIndex = getSelectedRewardIndexes(_rewardsElementMenu);
        ...
        /// <returns></returns>
        private List<Reward> getSelectedReward(MMenu menu)
        {
        ...
        /// </summary>
        /// <param name="menu">From this menu</param>
        /// <returns></returns>
        private List<int> getSelectedRewardIndexes(MMenu menu)
        {
            List<int> selectedIndex = new List<int>();

            List<MSelectable> elements = menu.getElements();

            for (int i = 0; i < elements.Count; i++)
            {
                if (elements[i].isSelected && elements[i].GetComponents<Reward>() != null)
                    selectedIndex.Add(i);
