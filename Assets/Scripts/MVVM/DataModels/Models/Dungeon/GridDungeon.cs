...
namespace RPG.DataModule
{
    public class GridDungeon
    {
        ...
        /// Find a index room
        /// </summary>
        /// <param name="room">The room to find</param>
        /// <returns> A flatten index</returns>
        public (int heightIndex, int widthIndex) findRoomIndex(Room room)
        {
            for (int i = 0; i < _rooms.GetLength(0); i++)
                for (int j = 0; j < _rooms.GetLength(1); j++)
                    if (_rooms[i, j] == room)
                        return (heightIndex: i, widthIndex: j);

            return (heightIndex: 0, widthIndex:0) ;
        }

        /// <summary>
        /// Find a room based on his height and width index
        /// </summary>
